...
class TicTacToeGame {
  ...
  public Mark getWinner() {
    ...
    for (List<Mark> combo : combos) {
      if (combo.get(0) != Mark.EMPTY && combo.get(0) == combo.get(1) && combo.get(1) == combo.get(2)) {
        return combo.get(0);
      }
    }
    return Mark.EMPTY;
  ...
  public void addMove(int position, Mark mark) {
    board.set(position - 1, mark);
  }
  ...
  public boolean isOver() {
    return getWinner() != Mark.EMPTY || getOpenPositions().isEmpty();
  }
